# -*- coding: utf-8 -*-
"""Generate Panduan Penggunaan Container Deposit Management v16.0.1.0.58 (.docx)."""
...
DOC_PATH = os.path.join(
    ...
    'Panduan_Penggunaan_Container_Deposit_Management_v16_1_0_58.docx',
)
...
title('Versi Modul: 16.0.1.0.58', 13, bold=False)
doc.add_paragraph()
...
title('Tanggal: 20 August 2026', 12, bold=False)
...
for item in [
    ...
    '2. Perubahan Penting Hingga Versi 16.0.1.0.58',
...
bullet('sequence_reset_period, ab_roman_sequence (bulan Romawi %(rom_month)s), ar_core, account_move_name_sequence')

# ============ 2 ============
h1('2. Perubahan Penting Hingga Versi 16.0.1.0.58')
table(['No', 'Perubahan', 'Keterangan'], [
    ...
    ['27', 'Register Payment wizard compat (v56)',
     'Override AccountPaymentRegister._create_payment_vals_from_wizard menerima batch_result opsional '
     'agar kompatibel dengan sr_manual_currency_exchange_rate (tanpa TypeError missing batch_result).'],
    ['28', 'Cancel = Cancelled (v57)',
     'Tombol Cancel mengembalikan status ke Cancelled (bukan Draft). Reopen untuk edit memakai Set to Draft.'],
    ['29', 'Deposit clearing saat Validate FTM LC (v58)',
     'Saat Validate FTM Landed Cost, CDM memposting journal companion: '
     'Dr Charge Account (6130006/6130016) | Cr Deposit Account (1720002/1720003) sebesar Final Deduction, '
     'lalu menautkannya ke settlement (Journal Entry). Tab Journal Items LC tetap menampilkan journal LC (JLC); '
     'clearing deposit dilihat di smart button Journal Entries CDM. '
     'Tombol Post Deposit Clearing tersedia pada LC yang sudah Validate tapi clearing belum ada.'],
])

# ============ 3 ============
h1('3. Prasyarat Instalasi & Master Data')
h2('3.1 Sebelum Install / Upgrade')
number('Pastikan modul custom_import, stock_landed_costs, sequence_reset_period, ab_roman_sequence, '
       'ar_core, dan account_move_name_sequence sudah terpasang.')
...
para('Pada Validate terbentuk DUA journal (sejak v58):', bold=True)
bullet('Journal A — Landed Cost (JLC/…): valuasi stok / HPP vs Charge Account (tampil di tab Journal Items LC).')
bullet('Journal B — Deposit Clearing: Dr Charge Account | Cr Deposit Account 1720002/1720003 '
      '(tampil di smart button Journal Entries pada form CDM; ditautkan ke settlement.journal_entry_id).')

para('Contoh Journal A — Validate LC Final Deduction Rp 1.900.000 (semua qty masih di gudang), '
...
table(['Akun', 'Kode', 'Debit', 'Credit', 'Keterangan'], [
    ...
    ['Biaya Sewa Container (Demurrage)', '6130006', '—', '1.900.000',
     'Contra akun charge LC; untuk jaminan container biasanya 6130016'],
])
...
para('Contoh Journal B — Deposit Clearing (otomatis setelah Validate, v58), Final Deduction Rp 1.900.000:')
table(['Akun', 'Kode', 'Debit', 'Credit', 'Keterangan'], [
    ['Biaya Sewa Container (Demurrage)', '6130006', '1.900.000', '—',
     'Charge Account settlement (6130016 bila jaminan container)'],
    ['Uang Muka Jaminan Sewa Container', '1720003', '—', '1.900.000',
     'Clear sisa aset deposit = Final Deduction (1720002 bila jaminan container)'],
])
para('Tanpa Journal B, akun 1720002/1720003 tetap menyisakan saldo sebesar Final Deduction '
     '(hanya Refund CN di Stage 9a yang mengurangi uang muka). Setelah Refund CN + Journal B, '
     'saldo uang muka terkait transaksi = 0.', italic=True)
para('Untuk LC yang sudah Validate sebelum upgrade v58: buka form Landed Cost → tombol '
     'Post Deposit Clearing (muncul selama clearing belum ada).', italic=True)
...
table(['Tombol', 'Terlihat di Status', 'Group', 'Fungsi / Validasi Utama'], [
    ...
    ['Create FTM Landed Cost', 'Approved (hijau setelah refund paid; polos sebelumnya)', 'Accounting',
     'Buat draft Landed Cost FTM; Compute diizinkan. Validate memposting Journal A (JLC) + Journal B (deposit clearing, v58)'],
    ...
    ['Cancel', 'Draft s.d. Deposit Paid & Done (disembunyikan di tahap settlement/Approved); '
     ...
     'Batalkan dokumen → status Cancelled (final, v57). Saat deposit bill Paid: diblokir. Setelah Reversed: tersedia untuk menutup CDM.'],
])
para('Administrator dapat melihat dan mengeksekusi seluruh tombol pada semua status.', italic=True)
para('Tombol Post Deposit Clearing muncul di form Landed Cost CDM (bukan header CDM) jika LC sudah Validate '
     'tetapi journal clearing deposit belum ada (v58).', italic=True)
...
table(['Smart Button', 'Muncul Jika', 'Fungsi'], [
    ...
    ['Journal Entries', 'journal_entry_count > 0',
     'Buka journal clearing deposit (Dr 613000x / Cr 172000x) yang ditautkan di settlement'],
])
...
para('Journal A — contoh valuasi LC (semua qty masih di gudang, Final Deduction Rp 1.900.000):')
table(['Akun', 'Kode', 'Debit', 'Credit', 'Keterangan'], [
    ['Persediaan Penjualan', '1300000', '1.900.000', '—', 'Naikkan nilai stok (porsi on-hand)'],
    ['Biaya Sewa Container (Demurrage)', '6130006', '—', '1.900.000', 'Contra charge LC (jaminan container: 6130016)'],
])
para('Jika sebagian qty sudah keluar, porsi terjual biasanya Debit 5201000 Pembelian Barang Penjualan (COGS).', italic=True)
para('Journal B — Deposit Clearing otomatis setelah Validate (v58):')
table(['Akun', 'Kode', 'Debit', 'Credit', 'Keterangan'], [
    ['Biaya Sewa Container (Demurrage)', '6130006', '1.900.000', '—', 'Charge Account settlement'],
    ['Uang Muka Jaminan Sewa Container', '1720003', '—', '1.900.000', 'Clear aset deposit = Final Deduction'],
])
para('Mapping Charge → Deposit: Sewa 6130006↔1720003; Jaminan Container 6130016↔1720002.', italic=True)
para('Tab Journal Items pada form LC hanya menampilkan Journal A. Journal B dilihat dari smart button '
     'Journal Entries pada form CDM, atau Accounting → Journal Items (ref/label CDM deposit clearing).', italic=True)
...
number('Jika Journal B (deposit clearing) sudah terposting, batalkan/balik juga journal tersebut agar 172000x tidak salah saldo.')
...
para('Contoh journal saat Reverse Landed Cost (Journal A):')
table(['Akun', 'Kode', 'Debit', 'Credit', 'Keterangan'], [
    ['Biaya Sewa Container (Demurrage)', '6130006', '1.900.000', '—', 'Balik contra charge LC'],
...
table(['Langkah', 'Membuat Journal?', 'Jenis Dokumen', 'Pola Jurnal (sederhana)'], [
    ...
    ['Create FTM LC (draft/Compute)', 'Tidak', 'stock.landed.cost draft', '—'],
    ['Validate FTM LC — Journal A', 'Ya', 'JLC/… (Landed Cost Journal)', 'Dr Stok 1300000 dan/atau HPP 5201000 | Cr 6130006/6130016'],
    ['Validate FTM LC — Journal B (v58)', 'Ya', 'Journal Entry clearing (link settlement)', 'Dr 6130006/6130016 | Cr 1720002/1720003 (= Final Deduction)'],
...
number('[Accounting] Create FTM Landed Cost Rp 2.500.000 → Compute → Validate → cek tab Journal Items (Journal A) '
       'dan smart button Journal Entries CDM (Journal B clearing deposit).')
...
table(['Event', 'Dampak'], [
    ...
    ['Validate Landed Cost — Journal A', 'Stok/HPP +2,5jt; Cr Charge Account 613000x 2,5jt'],
    ['Validate Landed Cost — Journal B (v58)', 'Dr Charge Account 2,5jt; Cr Uang Muka 2,5jt'],
...
h1('17. Checklist UAT')
for item in [
    'Upgrade modul ke 16.0.1.0.58 di DB test; akun 1720002 & 1720003 aktif; sequence_reset_period ≥ 15.0.1.0.1; ab_roman_sequence terpasang.',
    ...
    'Register Payment: draft bill auto-Post; Create Payment tidak error TypeError batch_result (v56).',
    'Register Payment tidak error KeyError rom_month (sequence %(rom_month)s dinormalisasi / ab_roman_sequence).',
    ...
    'Deposit Paid + bill Reversed (refund paid): Settlement & Set to Draft tersembunyi; Cancel tampil → status Cancelled.',
    ...
    'Form CN dari CDM tanpa CREATE STTF; Reset to Draft hanya satu tombol. Journal CN: Dr Hutang | Cr 172000x (bukan 613000x).',
    'Create FTM Landed Cost → Compute → Validate → tab Journal Items LC = Journal A (Dr Stok/HPP | Cr 613000x).',
    'Setelah Validate: Journal B clearing (Dr 613000x | Cr 172000x) muncul di Journal Entries CDM; saldo uang muka = 0.',
    'LC lama tanpa clearing: tombol Post Deposit Clearing tersedia hingga clearing terposting.',
...
table(['Item', 'Nilai'], [
    ...
    ['Versi modul', '16.0.1.0.58'],
    ['File panduan', 'Panduan_Penggunaan_Container_Deposit_Management_v16_1_0_58.docx'],
    ['Perubahan utama sejak 16.0.1.0.54',
     'v56: Register Payment wizard batch_result compat; '
     'v57: Cancel → Cancelled (bukan Draft); '
     'v58: Validate FTM LC memposting Journal B deposit clearing '
     '(Dr 6130006/6130016 | Cr 1720002/1720003) + tombol Post Deposit Clearing'],
    ['Format panduan', 'Microsoft Word (.docx)'],
    ['Bahasa', 'Indonesia'],
    ['Tanggal', '20 August 2026'],
])
para('Dokumen ini disusun berdasarkan evaluasi source code modul (models, views, hooks, security, report, '
     'migrations) hingga versi 16.0.1.0.58. Detail akun Debit/Kredit Landed Cost final dapat berbeda antar '
     ...
     'Tab Journal Items pada form LC menampilkan Journal A setelah Validate; Journal B clearing deposit '
     'ditampilkan via smart button Journal Entries pada form CDM.', italic=True)

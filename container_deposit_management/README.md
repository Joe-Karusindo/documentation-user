...
Version 16.0.1.0.56
-------------------
- Fix Register Payment wizard compatibility:
  `AccountPaymentRegister._create_payment_vals_from_wizard` now accepts
...
Version 16.0.1.0.60
-------------------
- Fix Accounting > Vendors > Bills showing only CDM bills after opening bills
  from a Container Deposit document.
- Root cause: `_open_moves` called `ir.actions.act_window.update()` on the shared
  `account.action_move_in_invoice_type`, permanently writing CDM domain/view_id.
- `_open_moves` now returns a transient action dict via `_for_xml_id` (no DB write).
- Upgrade migration restores the standard Vendor Bills action domain
  `[('move_type', '=', 'in_invoice')]` and default bill tree view when corrupted.

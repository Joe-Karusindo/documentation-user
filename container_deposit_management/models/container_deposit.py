...
class ImportContainerDeposit(models.Model):
    ...
    def _open_moves(self, moves, title, action_xmlid='account.action_move_in_invoice_type'):
        """Open Vendor Bills / Refunds for this CDM document only.

        IMPORTANT: never write to shared ``ir.actions.act_window`` records
        such as ``account.action_move_in_invoice_type``. Calling
        ``recordset.update(...)`` permanently corrupts Accounting > Vendors >
        Bills (domain becomes only the last CDM bill ids). Always return a
        transient action dict via ``_for_xml_id``.
        """
        action = self.env['ir.actions.actions']._for_xml_id(action_xmlid)
        moves = moves.exists()
        # CDM variant of the invoice form: no STTF buttons, single Reset to Draft.
        form = self.env.ref(
            'container_deposit_management.view_cdm_move_form',
        ...
        if len(moves) == 1:
            action.update({
                'name': title,
                'view_mode': 'form',
                'views': [(form.id, 'form')] if form else [(False, 'form')],
                'res_id': moves.id,
                'domain': [('id', '=', moves.id)],
            })
        else:
            ...
            action.update({
                ...
                'views': [
                    (tree.id, 'tree'),
                    (form.id, 'form') if form else (False, 'form'),
                ],
                'view_id': tree.id,
                'res_id': False,
            })
    ...
    def action_view_vendor_bills(self):
        ...
        return self._open_moves(
            self.deposit_line_ids.mapped('vendor_bill_id'),
            _('Vendor Bills'),
            action_xmlid='account.action_move_in_invoice_type',
        )

    def action_view_refund_bills(self):
        self.ensure_one()
        return self._open_moves(
            self.settlement_line_ids.mapped('refund_bill_id'),
            _('Vendor Credit Notes'),
            action_xmlid='account.action_move_in_refund_type',
        )
